...
public class CharDassaut implements Dessinable {
	...
	private final int DISTANCE_CHAR_VIE = 15;
	...
	public void dessiner(Graphics2D g2d, AffineTransform mat) {
		if (!mort) {
			...
			canon = new Rectangle2D.Double(x, y - getHauteurImgScaled() / 2 - LARGEUR_CANON, LONGUEUR_CANON,
					LARGEUR_CANON);
			vie = new Rectangle2D.Double(x - LONGUEUR_VIE / 2,
					y - getHauteurImgScaled() - LARGEUR_VIE - DISTANCE_CHAR_VIE,
					LONGUEUR_VIE - LONGUEUR_VIE / NOMBRE_POINT_VIE * compteurTouche, LARGEUR_VIE);
	...
	/**
	 * permet de comptabiliser le nombre de points de vie perdu et fait disparaitre
	 * le char quand il n'en a plus
	 */
	public void touche() {
		compteurTouche++;
	...
	public Area getAireChar(AffineTransform mat) {
		Rectangle2D.Double aire = new Rectangle2D.Double(x - getLargeurImgScaled() / 2, y - getHauteurImgScaled(),
				getLargeurImgScaled(), getHauteurImgScaled());
