...
public class ChampElectrique implements Dessinable {
	...
	public Vecteur getForceElectrique(Projectile projectile) {
		...
		if (distance.module() - 5 > INCREMENT_CERCLE * NB_DE_CERCLE / 2.0) {
			...
			try {
				...
				// Les charges sont en microcoulombs et les calculs sont en coulombs. Ainsi, il
				// faut multiplier chacune des charges par Math.pow(10,-6)
				Vecteur forceElec = rChapeau.multiplie(CONST_K * charge
						* (Math.pow(10, -3) * chargeProjectile * (Math.pow(10, -3) / Math.pow(distance.module(), 2))));
	...
	/**
	 * Méthode qui permet de fixer la charge de la charge ponctuelle qui génère le champ électrique
	 * @param charge  la charge de la charge ponctuelle qui génère le champ électrique en milicoulomb
	 */
